...
typedef enum { ST_DATA_EXIT = 0, ST_DATA_INIT , ST_ENVIAR_FRAME, ST_PROCESAR_RESPUESTA } t_data_state;

static t_data_state tkComms_data_state;
static uint8_t intentos;
static st_dataRecord_t dataRecord;

#define MAX_INTENTOS_ENVIAR_DATA_FRAME	4

#define AC_init_for_tx() ( intentos =  MAX_INTENTOS_ENVIAR_DATA_FRAME )

static t_data_state tkComms_st_data_init(void);
static t_data_state tkComms_st_data_enviar_frame(void);
static t_data_state tkComms_st_data_procesar_respuesta(void);

static bool EV_hay_datos_para_transmitir(void);
static bool EV_envio_frame_OK(void);
...
// ENTRY
	...
	tkComms_data_state = ST_DATA_INIT;
	u_debug_printf_P( DEBUG_COMMS, PSTR("COMMS:Data.\r\n\0"));
...
// LOOP
	for( ;; )
	{
		...
		switch ( tkComms_data_state ) {
		case ST_DATA_INIT:
			// Determina si hay datos o no para transmitir
			tkComms_data_state = tkComms_st_data_init();
			break;
		case  ST_ENVIAR_FRAME:
			tkComms_data_state = tkComms_st_data_enviar_frame();
			break;
		case ST_PROCESAR_RESPUESTA:
			tkComms_data_state = tkComms_st_data_procesar_respuesta();
			break;
		case ST_DATA_EXIT:
			// Salgo del estado.
			return(ST_STANDBY);
...
t_data_state next_state;
	...
	if ( EV_envio_frame_OK() ) {

		next_state = ST_PROCESAR_RESPUESTA;
...
t_data_state next_state;
	...
	if ( EV_procesar_respuesta_OK() ) {
		...
		next_state = ST_DATA_INIT;

	} else {
		// Error al procesar: reintento enviar el frame
		next_state = ST_ENVIAR_FRAME;
	}
...
FAT_t fat;
	...
	if ( fat.rcds4rd > 0) {
		...
		retS = false;
		u_debug_printf_P( DEBUG_COMMS, PSTR("COMMS: bd EMPTY\r\n\0"));
	}
...
FAT_t fat;
	...
	xCOMMS_send_P( PSTR("CTL:%d;\0"),fat.rdPTR );
	xCOMMS_send_dr(&dataRecord);
...
FAT_t fat;
	...
	while (  u_check_more_Rcds4Del(&fat) ) {
		...
		recds_borrados++;
		FAT_read(&fat);
		u_debug_printf_P( DEBUG_COMMS, PSTR("COMMS: mem wrPtr=%d,rdPtr=%d,delPtr=%d,r4wr=%d,r4rd=%d,r4del=%d \r\n\0"), fat.wrPTR, fat.rdPTR, fat.delPTR, fat.rcds4wr, fat.rcds4rd, fat.rcds4del );
	}
