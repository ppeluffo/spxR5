...
uint8_t timeout;
	...
	if (retS) {
		xprintf_PD( f_debug,  PSTR("COMMS: gprs ATI OK [%s] en [%d] secs\r\n\0"), gprs_status.buff_gprs_imei, timeout );
	} else {
		gprs_AT();
...
bool retS = false;
	...
	if ( retS ) {
		xprintf_PD( f_debug,  PSTR("COMMS: gprs IPADDR [%s] OK\r\n\0"), ip_assigned );
	} else {
		gprs_AT();
...
uint8_t intentos;
	...
	// Tengo la IP. La leo.
	gprs_IPADDR(scan_boundle->f_debug, NULL);
